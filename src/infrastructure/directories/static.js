const { v4: uuid } = require('uuid');

const getPageOfUsers = async (pageNumber, pageSize, includeDevices, includeCodes, changedAfter, correlationId) => {
  const result = {
    users: [
      {
        sub: '7a1b077a-d7d4-4b60-83e8-1a1b49849510',
        given_name: 'Test',
        family_name: 'Tester',
        email: 'test@localuser.com',
        password: '',
        salt: 'PasswordIs-password-',
        devices: [
          { id: '6eebc499-e69e-4556-95e5-dc0300c12748', type: 'digipass', serialNumber: '1234567890' },
        ],
      },
    ],
    numberOfPages: 1,
  };
  if (!includeDevices) {
    result.users[0].devices = undefined;
  }
  return Promise.resolve(result);
};

const getUser = async (uid, correlationId) => ({
  sub: '7a1b077a-d7d4-4b60-83e8-1a1b49849510',
  given_name: 'Test',
  family_name: 'Tester',
  email: 'test@localuser.com',
  password: '',
  salt: 'PasswordIs-password-',
});

const getPageOfInvitations = async (pageNumber, pageSize, changedAfter, correlationId) => ({
  invitations: [],
  numberOfPages: 1,
  page: pageNumber,
});

const getInvitation = async (invitationId, correlationId) => ({
  firstName: 'Some',
  lastName: 'User',
  email: 'some.user@test.local',
  keyToSuccessId: '1234567',
  tokenSerialNumber: '12345678901',
  id: invitationId,
});

const getUserDevices = async (uid, correlationId) => [
  {
    id: '6eebc499-e69e-4556-95e5-dc0300c12748',
    type: 'digipass',
    serialNumber: '1234567890',
  },
];

const getUserAssociatedToDevice = async (type, serialNumber, correlationId) => Promise.resolve(null);

const updateUser = async (uid, givenName, familyName, correlationId) => Promise.resolve();

const deactivate = async (uid, correlationId) => Promise.resolve();

const reactivate = async (uid, correlationId) => Promise.resolve();

const createInvite = async (givenName, familyName, email, digipassSerialNumber, clientId, redirectUri, correlationId) => Promise.resolve(uuid());

const updateInvite = async (id, email, correlationId) => Promise.resolve(null);

const deactivateInvite = async (inviteId, reason, correlationId) => Promise.resolve(uuid());

const reactivateInvite = async (inviteId, reason, correlationId) => Promise.resolve(uuid());

const createUserDevice = async () => Promise.resolve();

const createChangeEmailCode = async (userId, newEmailAddress, clientId, redirectUri, correlationId) => Promise.resolve({});

const getChangeEmailCode = async (userId, correlationId) => Promise.resolve(null);

const deleteChangeEmailCode = async (userId, correlationId) => Promise.resolve();

const getUsersById = async (userIds, correlationId) => Promise.resolve([]);

const getUsersByIdV2 = async (userIds, correlationId) => Promise.resolve([]);

const getLegacyUsernames = async (userIds, correlationId) => Promise.resolve([]);

module.exports = {
  getPageOfUsers,
  getUser,
  getPageOfInvitations,
  getInvitation,
  getUserDevices,
  getUserAssociatedToDevice,
  updateUser,
  deactivate,
  reactivate,
  createInvite,
  updateInvite,
  deactivateInvite,
  reactivateInvite,
  createUserDevice,
  createChangeEmailCode,
  getChangeEmailCode,
  deleteChangeEmailCode,
  getUsersById,
  getLegacyUsernames,
  getUsersByIdV2,
};
