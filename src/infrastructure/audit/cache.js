...
const readUserStatsFromStore = async () => new Promise((resolve, reject) => {
...
const getDateOfLastAuditRecord = async () => Promise.resolve(dateOfLastAuditUpdate);
...
const getStatsForUser = async (userId) => {
  const stats = userStats.find((u) => u.userId.toLowerCase() === userId.toLowerCase());
