...
const buildUser = async (user, allDevices, correlationId) => {
  ...
  return userDevices.map((device) => {
    if (allDevices.find((aDevice) => aDevice.serialNumber === device.serialNumber)) {
      allDevices.find((aDevice) => aDevice.serialNumber === device.serialNumber).isAssigned = true;
    }
...
const buildDevicesWithoutUser = (devices) => devices.map((device) => ({
  id: uuid(),
  name: undefined,
  email: undefined,
...
const syncUserDevicesView = async () => {
  ...
  while (hasMorePages) {
    ...
    if (pageOfUsers.users) {
      const mappedUsers = await asyncMapLimit(pageOfUsers.users, async (user) => await buildUser(user, allDevices, correlationId));
  ...
  if (allDevices && allDevices.length > 0) {
    const devicesWithoutUsers = buildDevicesWithoutUser(allDevices.filter((device) => device.isAssigned === undefined));
