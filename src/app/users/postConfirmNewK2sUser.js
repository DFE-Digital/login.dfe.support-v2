...
const postConfirmNewK2sUser = async (req, res) => {
  ...
  const invitationId = await createInvite(
    req.session.k2sUser.firstName,
    req.session.k2sUser.lastName,
    req.session.k2sUser.email,
    req.session.digipassSerialNumberToAssign,
    keyToSuccessOrigin.clientId,
    keyToSuccessOrigin.redirectUri,
    req.id,
  );
  ...
  logger.audit(
    `${req.user.email} (id: ${req.user.sub}) invited ${req.session.k2sUser.email} to user key-to-success. Key-to-Success id: ${req.session.k2sUser.k2sId}, Digipass: ${req.session.digipassSerialNumberToAssign}`,
    {
      type: 'support',
      subType: 'user-invite',
      userId: req.user.sub,
